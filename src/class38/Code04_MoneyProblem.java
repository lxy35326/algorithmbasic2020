...
public class Code04_MoneyProblem {
    ...
    public static long proce(int[] d, int[] p, int ability, int index) {
        if (index == d.length) {
            return 0;
        }
        if (ability >= d[index]) {
            return Math.min(p[index] + proce(d, p, ability + d[index], index + 1),
                    proce(d, p, ability, index + 1));
        } else {
            return p[index] + proce(d, p, ability + d[index], index + 1);
        }
    }
    ...
    //dp[i][j]代表，从i位置开始往后算（前面的已经计算完），目前的能力是p，至少需要多少钱
    public static long fun(int[] d, int[] p) {
        int len = d.length;
        int r = 0;
        for (int i : d) {
            r += i;
        }
        int[][] arr = new int[len + 1][r + 1];
        for (int i = d.length - 1; i > -1; i--) {
            for (int j = 0; j < r + 1; j++) {
//				if (d[i] + j > r) continue;
                if (j < d[i]) {
                    arr[i][j] = p[i] + arr[i + 1][d[i] + j];
                } else {
                    arr[i][j] = Math.min(arr[i + 1][j], p[i] + arr[i + 1][d[i] + j]);
                }
            }
        }
        return arr[0][0];
    ...
    public static long fun2(int[] d, int[] p) {
        ...
        long[][] dp = new long[d.length][r + 1];
        //dp[i][j]代表：能够到达i位置,恰好花j元的情况下，能够获得的最大能力是多少。
        Arrays.fill(dp[0], -1);
        dp[0][p[0]] = d[0];
...
//		return proce(d, p, 0, 0);
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testTimes; i++) {
            ...
            if (ans1 != ans2 || ans2 != ans3 || ans1 != ans4 || ans1 != ans) {
